...
local utf8_explode_cache_string = nil; -- 09-12-2022: cache string
local utf8_explode_cache_table = nil; -- 09-12-2022: cache table
...
-- @return table
local function utf8_explode( s )-- 09-12-2022: improved performance by using variable bytes set with all bytes of the string in one shot and last string/table cache
	if (s == utf8_explode_cache_string) then -- 09-12-2022: same as last string?
	    return utf8_explode_cache_table; -- 09-12-2022: reuse last table
	end
	...
	local i = 1
	local bytes = {S.byte(s, 1, -1)} -- 09-12-2022: bytes is set with all bytes of the string in one shot
	local l = #bytes
	local cp, b, b2, trail
	local min
	while i <= l do
		b = bytes[i] -- 09-12-2022: retrieve byte in position i
		if b < 0x80 then
		...
		elseif b == 0xf4 then
			...
			if bytes[i + 1] > 0x8f then -- 09-12-2022: retrieve byte in position i + 1
				return nil
			end
			trail = 3
			cp = 4
		...
		for j = i + 1, i + trail do
			b = bytes[j] -- 09-12-2022: retrieve byte in position j
	...
	utf8_explode_cache_string = s; -- 09-12-2022: store last string
	utf8_explode_cache_table = ret; -- 09-12-2022: store last table
	return ret
end
