...
public final class Ifeq extends ParserFunction {

    public final static ParserFunction Instance = new Ifeq();

    @Override
	public String evaluate(WikiPage wp, ArrayList<String> parameters, Frame parent) {
        if (parameters.size() > 2) {
			TemplateParser tp = wp.getTemplateParser();
			String first = tp.parseParameter( parameters.get(0), wp, parent);
			String second = tp.parseParameter( parameters.get(1), wp, parent);
			boolean test;
			try {
				test = Double.parseDouble(first) == Double.parseDouble(second);
			} catch (NumberFormatException e) {
				test = first.equals(second);
			}
			if (test) {
				return tp.parseParameter( parameters.get(2), wp, parent);
			} else if (parameters.size() > 3) {
				return tp.parseParameter( parameters.get(3), wp, parent);
			}
		}
        return ""; 
	}
