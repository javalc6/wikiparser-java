...
public final class Fullurl extends ParserFunction {
	...
	public String evaluate(WikiPage wp, ArrayList<String> parameters, Frame parent) {
		...
		if (parameters.size() == 1) {
			return "//" + lang + ".wiktionary.org/wiki/" + encodeUrl(tp.parseParameter( parameters.get(0), wp, parent));
		} else if (parameters.size() > 1) {
				return "//" + lang + ".wiktionary.org/w/index.php?title=" + encodeUrl(tp.parseParameter( parameters.get(0), wp, parent))
