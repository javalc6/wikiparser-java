...
public final class Iferror extends ParserFunction {

    public final static ParserFunction Instance = new Iferror();

    @Override
	public String evaluate(WikiPage wp, ArrayList<String> parameters, Frame parent) {
        if (parameters.size() > 1) {
			TemplateParser tp = wp.getTemplateParser();
			if (tp.parseParameter( parameters.get(0), wp, parent).contains(" class=\"error\"")) {
